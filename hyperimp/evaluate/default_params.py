...
def find_default(alg, df, ranges):
    ...
    alg : Alg
        object of Alg class
    df : DataFrame
        df with top n datasets generated by get_topn()
    ranges : boolean
        store lower and upper values of bins (True) or just the average (False)
        ...
        dictionary with best parameter setting for an algorithm
            ...
            value: dict with
                    ...
                    value : (lower, upper, average)
                        ...
                        if ranges is False, only the average is returned
    """
    settings = {}
    ...
    for task_id in alg.perf['task_id'].unique():
        ...
        for param in alg.params:
            ...
            if param.boolean or param.nom:
                ...
                setting_average = (setting_lower + setting_upper)/2
                if param.intg:
            ...
            if ranges:
                ...
                task_settings[param.name] = setting_average
        settings[task_id] = task_settings
    return settings

def default_score(alg, topn, def_params):
    """
    For each hyperparameter, computes the percentage of topn data points for which 
    the value falls within the range of the default parameter. 
    
    Parameters
    ----------
    alg : alg object
        generate with init_algs()
    topn : dataframe
        generate with get_topn()
    def_params : dict
        generate with find_defaults(ranges = True)
    """
    num_total = len(topn)
    scores = {}
    for parameter in alg.params:
        num_right = 0
        name = parameter.name
        for i in topn.iterrows():
            row = i[1]
            value = row[name]
            task_id = row['task_id']
            lower, upper, average = def_params[task_id][name]
            if parameter.nom or parameter.boolean:
                if value == lower:
                    num_right += 1
            else:
                if (value >= lower) and (value <= upper):
                    num_right +=1
        scores[name] = num_right/num_total
    return scores
