...
def run_experiment(rscv, task, args):
    try: 
        ...
        while count <= 100:
            try:
                print("%s Started classifier %s, condition %s, parameters '%s', deftype '%s', RS seed %s on task %s, dataset '%s'." % (hyperimp.utils.get_time(), args.classifier, args.condition, args.params, args.deftypes, args.seed, args.task_id, task.get_dataset().name))
                # train model
        ...
        if args.condition == "fixed":
            ...
            for param, deftype in zip(args.params.split(), args.deftypes.split()):
                if deftype == 'sklearn':
                    ...
                    def_param = def_params[args.classifier][args.task_id][param]
                    
                #prevent JSON serializable error
                if type(def_param) == np.bool_: 
                    def_param = bool(def_param)
                    
                #turn float into int
                if ((param == 'min_samples_leaf') or (param == 'min_samples_split')):
                    def_param = int(def_param)
                
                def_params_list.append(def_param)
